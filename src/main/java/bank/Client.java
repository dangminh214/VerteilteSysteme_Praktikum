...
public class Client extends Thread {
  ...
  private Bank bank;
  public Client(Socket clientConnection, Bank bank)
      throws IOException {
    this.bank= bank;
    ...
    try {
      ...
      while (!connection.isClosed()) {
        ...
        if (bodyContent == null|| bodyContent.equals(""))
          ...
          int money = 0;
          CodeOfWertpapier codeOfWertpapier = null;
          int quantity =0;
          ...
          for (String parameter : parameters) {
            ...
            if(keyValue[0].equals("transactionType")&& keyValue.length==2){
              ...
              codeOfWertpapier = getCode(keyValue[1]);
            }
          ...
          evaluateRequest(transactionType,money, codeOfWertpapier,quantity);
  ...
  private void evaluateRequest(String transactionType, int money, CodeOfWertpapier codeOfWertpapier, int quantity)
      throws IOException {
    if(transactionType.equals("")) return;
      ...
      Wertpapier wertpapier = bank.getSavedMessage().get(codeOfWertpapier);
      if(wertpapier == null){
        this.statusMessage= DENY_ANSWER;
        sendAnswer("No Stock code found");
      }
      else{
        if(wertpapier.getQuantity()< quantity){
          this.statusMessage= DENY_ANSWER;
          sendAnswer("Not enough Stock with this code");
        }
        else{
          wertpapier.setQuantity(wertpapier.getQuantity()-quantity);
          this.statusMessage= DEFAULT_ANSWER;
